...
def main(args):
    ...
    if args.resume:
        ...
        ###
        if args.fine_tune or args.transfer_learning or args.pretrained_encoder:
    ...
    if args.pretrained_encoder:
        ...
        # make a list of params with .requires_grad == True
        params_to_update = [
            {"params": [p for n, p in model_without_ddp.named_parameters() if "backbone" not in n and p.requires_grad]},
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "backbone" in n and p.requires_grad],
    ...
    if args.transfer_learning:
        # initially, set gradient computation to False for all params
        ...
        # enable gradient computation only for newly added params
        for name,param in model.named_parameters():
            if name == "class_embed.weight" or name == "class_embed.bias":
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            ###
            # changed to checkpoint every 10 epochs
            if (epoch + 1) % 10 == 0:
                checkpoint_paths.append(output_dir / f'checkpoint{epoch:04}.pth')
            ###
